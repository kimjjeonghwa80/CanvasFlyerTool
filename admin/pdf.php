<?php
			...
			foreach ($objects->objects as $object) {
				...
				switch ($object->type) {
					...
					case 'textbox':
						...
						if($fontFamily == 'Arial' && $object->fontWeight == 'bold') {
							$fontpath = "./tcpdf/fonts/googlefonts/" . str_replace(" ","_", $fontFamily) . "/" .  str_replace(" ","", $fontFamily) ."-Bold.ttf";
						}

						if($fontFamily == 'Arial' && $object->fontStyle == 'italic') {
							$fontpath = "./tcpdf/fonts/googlefonts/" . str_replace(" ","_", $fontFamily) . "/" .  str_replace(" ","", $fontFamily) ."-Italic.ttf";
						}

						if($fontFamily == 'Arial' && $object->fontWeight == 'bold' && $object->fontStyle == 'italic') {
							$fontpath = "./tcpdf/fonts/googlefonts/" . str_replace(" ","_", $fontFamily) . "/" .  str_replace(" ","", $fontFamily) ."-BI.ttf";
						}
	...
	$pdf->Close();
	$filename = $_SERVER['DOCUMENT_ROOT'] . "NewHTML5CanvasFlyerTool/admin/outputpdfs/" . $_POST['filename'];
